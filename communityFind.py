...
# pos1=sfdp_layout(G)
begin=time.time()
# part= community_structure(G, 1000,cnum,t_range=(5, 0.1))
max_B=30
min_B=10
state=minimize_blockmodel_dl(G,max_B=30,min_B=10)
part=state.b
partitions=[0 for i in range(pnum)]
cnum=0
for v in G.vertices():
    partitions[int(v)]=part[v]
    cnum=max(cnum,part[v])
cnum+=1
...
output=open(outputfolder+'/'+pname+'treeArray'+str(evt)+ 'pnum'+str(pnum)+\
    'cnum'+str(cnum)+'edges'+str(enum)+'rpnum'+str(repost_num)+'sto'+'.json','w')
